...
1. Figure out which dev server to verify against — **a worktree must run its own server, not the primary checkout's**:
   ...
   - **In the primary checkout** with a server already running for this directory: reuse it. Find its port from `lsof -iTCP -sTCP:LISTEN -P | grep node` (default 4321). Do NOT kill it — Ben previews changes live via HMR.
